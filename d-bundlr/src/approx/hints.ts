import {ReadHint, HintsJSON, WriteHint, RequireHint, EvalHint, CallHint} from "../typings/hints";
...
export class Hints {
    ...
    readonly calls = new Map<LocationJSON, Array<CallHint>>();
    ...
    addCallHint(h: CallHint) {
        mapArrayAddNoDuplicates(h.loc, h, this.calls, (v1: CallHint, v2: CallHint) => {
            if (v1.loc===v2.loc && v1.base===v2.base && v1.function===v2.function && v1.args.length===v2.args.length) {
                for (let i = 0; i < v1.args.length; i++) {
                    if (v1.args[i] !== v2.args[i])
                        return false;
                }
                return true;
            }
    ...
    toJSON(): HintsJSON {
        return {
            ...
            calls: Array.from(this.calls.values()).flat()
        }
    }
