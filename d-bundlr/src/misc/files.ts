...
function* expandRec(path: string, sub: boolean, visited: Set<string>): Generator<string> {
    try {
        ...
        logger.info(`Info: Unable to read ${path}`);
    }
}
 ...
 */
export function codeFromLocation(loc: Location | null | undefined, contentLength: number = 50): string {
    if (!loc)
        return "-";
    ...
    if (!content) {
        content = "";
        if (loc && (loc.module|| Object.keys(loc).includes("filename"))) {
            const fileContent = readFileSync(loc.module?loc.module.getPath():(<any>loc).filename, 'utf-8').toString().split(/\r?\n|\r/);
