...
import {unbundle} from "./unbundle/unbundle";
import {unpackFiles} from "./unbundle/sourcemapping/unpackBySourceMap";
...
program
    ...
    .option("--debundle-dir <directory>", "directory for debundled files")
    .option("--sound-mode", "soundness mode for debundle(default: false)")
    .option("--no-predict", "disable prediction of debundled module and functions")
    .option("--no-machine-learning", "disable machine learning for prediction")
    .option("--use-source-map", "unpack files by sourcemap")
    .usage("[options] [files]")
    .addHelpText("after",
    ...
    if (options.dynamic) {
        ...
        if (options.approxOnly) {
            ...
            if (options.useSourceMap) {
                unpackFiles(files, options.debundleDir, options.basedir);
            } else {
                try {
                    let {mains, chunks, errors, success, wrappers, functions, requireFunctionPredictedBy, failedFiles} = await unbundle(files);
                    logger.info(JSON.stringify({"success": true, "error": "", mainFiles: mains.length, chunks: chunks.length, errors: errors.length, succ_runs: success.length, wrappers: Array.from(wrappers), functions: Array.from(functions), requireFunctionPredictedBy, failedFiles}, null, 2));
                    if (options.diagnosticsJson)
                        writeFileSync(options.diagnosticsJson, JSON.stringify({"success": true, "error": "", mainFiles: mains.length, chunks: chunks.length, errors: errors.length, succ_runs: success.length, wrappers: Array.from(wrappers), functions: Array.from(functions), requireFunctionPredictedBy, failedFiles}))
                } catch (e) {
                    if (options.diagnosticsJson) {
                        logger.error(options.diagnosticsJson);
                        logger.error(e instanceof Error? e.stack??e:e);
                        writeFileSync(options.diagnosticsJson, JSON.stringify({"success": false, "error": e?e.toString():"Unknown error", mainFiles: 0, chunks: 0, errors: 0, succ_runs: 0}))
                        writeFileSync(options.diagnosticsJson+".error", e instanceof Error? e.stack??"unknown":"unknown");
                    }
                }
                process.exit(0);
            }
