...
import {codePreview, generateUid, memberExpressionWithProperty, Transform} from "./misc";
...
export default {
    ...
    visitor: () => {
        ...
        const fragmentType = memberExpressionWithProperty('Fragment');
        ...
        const jsxMatcher = m.callExpression(
            memberExpressionWithProperty(jsxFunction),
        ...
        return {
            CallExpression: {
                exit(path) {
                    ...
                    try {
                        attributes = convertAttributes(props.current!);
                    } catch (e) {
                        return;
                    }
