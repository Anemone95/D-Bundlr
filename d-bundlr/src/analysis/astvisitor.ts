...
import {Location, locationToStringWithFile, mapArrayAdd} from "../misc/util";
...
import {
    getClass,
    getEnclosingFunction, getEnclosingNonArrowFunction,
    getExportName,
    getImportName,
    getKey,
    getProperty,
...
export function visit(ast: File, op: Operations) {
    ...
    traverse(ast, {
            ...
            if (!options.oldobj) {
                ...
                if (encl) {
                    if (isFunction(encl.node)) {
                        ...
                        // collect uses of 'this' in arrow functions
                        const enclNonArrow = getEnclosingNonArrowFunction(path);
                        for (let c = path.getFunctionParent(); enclNonArrow && c && isArrowFunctionExpression(c.node); c = c.getFunctionParent()) {
                            const cf = a.functionInfos.get(c.node);
                            assert(cf);
                            if (logger.isDebugEnabled())
                                logger.debug(`Read 'this' in arrow function ${cf} at ${locationToStringWithFile(path.node.loc)}`);
                            cf.freeVarsReadThis = enclNonArrow;
                        }
                    } else {
        ...
        Identifier(path: NodePath<Identifier>) {
            ...
            if (binding) { // if no binding and not 'arguments', it's not an identifier use, so ignore

                if (options.variableKinds)
                    (binding.identifier as any)[IDENTIFIER_KIND] = binding.kind;

                // collect free variables
                if (path.node !== binding.identifier && !((binding.identifier.loc as Location).native)) {
                    ...
                    assert(isFunction(bindingFunction) || bindingFunction === undefined);
                    for (let c = path.getFunctionParent(); c && c.node !== bindingFunction; c = c.getFunctionParent()) {
                        const cf = a.functionInfos.get(c.node);
                        assert(cf);
                        const write = (isAssignmentExpression(path.parent) || isAssignmentPattern(path.parent)) && path.parent.left == path.node;
                        if (logger.isDebugEnabled())
                            logger.debug(`Free variable ${write ? "write" : "read"} '${binding.identifier.name}' in ${cf} at ${locationToStringWithFile(path.node.loc)}`);
                        (write ? cf.freeVarsWrite : cf.freeVarsRead).add(binding.identifier);
                    }
                }
            } else if (path.node.name === "arguments") {
                vp.registerArguments(path);

                // collect uses of 'arguments' in arrow functions
                const enclNonArrow = getEnclosingNonArrowFunction(path);
                for (let c = path.getFunctionParent(); enclNonArrow && c && isArrowFunctionExpression(c.node); c = c.getFunctionParent()) {
                    const cf = a.functionInfos.get(c.node);
                    assert(cf);
                    if (logger.isDebugEnabled())
                        logger.debug(`Free variable read 'arguments' in arrow function ${cf} at ${locationToStringWithFile(path.node.loc)}`);
                    cf.freeVarsReadArguments = enclNonArrow;
                }
            }
        },
        ...
        Function: { // FunctionDeclaration | FunctionExpression | ObjectMethod | ArrowFunctionExpression | ClassMethod | ClassPrivateMethod
            enter(path: NodePath<Function>) {
                if (options.debundleDir) {
                    ...
                    do {
                        if (p && (isFunctionDeclaration(p.node) || isObjectMethod(p.node) || isClassMethod(p.node) || isClassPrivateMethod(p.node))) {
                            func = p.node;
                        }
                        p = p?.parentPath;
                    } while (p);
                    if (func)
                        solver.globalState.outmostFunctions.add(func);
                }
        ...
        ArrayExpression(path: NodePath<ArrayExpression>) {
            ...
            if (!isParentExpressionStatement(path)) {
                ...
                let indexKnown = path.node.elements.length <= Infinity; // using ARRAY_UNKNOWN if more than 10 elements
