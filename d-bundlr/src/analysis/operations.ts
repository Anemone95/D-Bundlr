...
export class Operations {
    ...
    callFunction(path: CallNodePath) {
        ...
        if (p.isMemberExpression() || p.isOptionalMemberExpression()) {
            ...
            if(options.debundleDir && calleeVar) {
                this.solver.fragmentState.registerCallSites(path.node, baseVar, prop, calleeVar, argVars, getEnclosingFunction(path));
            }
        } else {
            if(options.debundleDir && calleeVar) {
                this.solver.fragmentState.registerCallSites(path.node, undefined, undefined, calleeVar, argVars, getEnclosingFunction(path));
            }
        }
    ...
    callFunctionBound(
        ...
        if (options.debundleDir) {
            if (t instanceof NativeObjectToken) {
                // logger.warn(`Call to native ${t} at ${locationToStringWithFileAndEnd(pars.node.loc)} in context ${context}`);//XXX
            ...
            } else if (isObjectPropertyVarObj(t) && calleeVar){
                f.addToken(t, f.getRepresentative(calleeVar));
            }
        }
        if (base)
    ...
    requireModule(str: string, resultVar: ConstraintVar | undefined, path: NodePath): ModuleInfo | DummyModuleInfo | undefined { // see requireModule in modulefinder.ts
        ...
        if (Module.isBuiltin(str)) {
            ...
            try {
                ...
                let filepath = requireResolve(str, this.file, f.a, path.node, f);
                // ad-hoc fix for cjs/mjs
                if (isImportDeclaration(path.node) && filepath) {
                    if (fs.existsSync(filepath.replace(/\/cjs\/index.js$/, "/react-redux.mjs")))
                        filepath = filepath.replace(/\/cjs\/index.js$/, "/react-redux.mjs");
                    if (fs.existsSync(filepath.replace(/\.js$/, ".module.js")))
                        filepath = filepath.replace(/\.js$/, ".module.js");
                    if (fs.existsSync(filepath.replace(/\.js$/, ".mjs")))
                        filepath = filepath.replace(/\.js$/, ".mjs");
                    if (fs.existsSync(filepath.replace(/\/cjs\//, "/esm/"))) {
                        filepath = filepath.replace(/\/cjs\//, "/esm/");
                    }
                    if (fs.existsSync(filepath.replace(/\/src\//, "/es/"))) {
                        filepath = filepath.replace(/\/src\//, "/es/");
                    }
                    if (fs.existsSync(filepath.replace(/\.cjs\./, ".es."))) {
                        filepath = filepath.replace(/\.cjs\./, ".es.");
                    }
                    if (fs.existsSync(filepath.replace(/eta\.umd\.js/, "browser.module.mjs"))) {
                        filepath = filepath.replace(/eta\.umd\.js/, "browser.module.mjs");
                    }
                    if (fs.existsSync(filepath.replace(/\/lib\//, "/es/"))) {
                        filepath = filepath.replace(/\/lib\//, "/es/");
                    }
                    if (fs.existsSync(filepath.replace(/\.js$/, ".m.js"))) {
                        filepath = filepath.replace(/\.js$/, ".m.js");
                    }
                    if (fs.existsSync(filepath.replace(/\/node\//, "/dist/"))) {
                        filepath = filepath.replace(/\/node\//, "/dist/");
                    }
                    if (fs.existsSync(filepath.replace(/\/commonjs\//, "/esm/"))) {
                        filepath = filepath.replace(/\/commonjs\//, "/esm/");
                    }
                }
                if (filepath) {
                    // TODO: should remove this
                    if (fs.existsSync(filepath.replace("b64u/dist/b64u.js","b64u/dist/b64u.m.js"))) {
                        filepath = filepath.replace("b64u/dist/b64u.js","b64u/dist/b64u.m.js");
                    }
